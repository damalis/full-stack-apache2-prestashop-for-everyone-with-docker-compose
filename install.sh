...
echo "======================================================================="
echo "|                                                                     |"
echo "|    full-stack-apache2-prestashop-for-everyone-with-docker-compose   |"
echo "|                         by Erdal ALTIN                              |"
echo "|                                                                     |"
echo "======================================================================="
sleep 2
...
read -p 'Enter Database Table Prefix(at least 3 characters, default : ps_): ' db_table_prefix
: ${db_table_prefix:=ps_}
...
if [ -x "$(command -v docker)" ] && [ "$(docker compose version)" ]; then
	...
	# installing PrestaShop and the other services
	docker compose up -d & export pid=$!
	echo "PrestaShop and the other services installing proceeding..."
	echo ""
	wait $pid
	if [ $? -eq 0 ]
	then
		...
		echo ""
		echo "Error! could not installed PrestaShop and the other services with docker compose" >&2
		exit 1
	fi
else
	echo ""
